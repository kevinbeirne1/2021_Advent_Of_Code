...
class ScoreLine:
    ...
    def __init__(self, nav_strings):
        self.strings_list = nav_strings
        self.error_score = 0
        self._completion_scores = []
        self.parse_lines()

    @property
    def completion_score(self):
        return 0 if not self._completion_scores else median(self._completion_scores)

    def parse_line(self, bracket_string):
        unclosed = []
        completion_score = 0
        valid_string = True
        for char in bracket_string:
            if char in "([{<":
                ...
                self.error_score += self.error_cost[char]
                valid_string = False
                break
            else:
                last_open = unclosed.pop()
                expected_bracket = self.inverse_brackets[last_open]
                if char != expected_bracket:
                    self.error_score += self.error_cost[char]
                    valid_string = False
                    break
        if valid_string:
            for char in unclosed[::-1]:
                completion_score *= 5
                completion_score += self.completion_cost[char]
        self._completion_scores += [completion_score] if completion_score else []

    def parse_lines(self):
        for bracket_string in self.strings_list:
            bracket_string = bracket_string.strip()
            self.parse_line(bracket_string)
...
class TestScoreLine(unittest.TestCase):
    ...
    def test_error_score(self):
        for test_case, expected_result in self.error_test_cases:
            with self.subTest():
                scored_line = ScoreLine(test_case)
                actual = scored_line.error_score
                self.assertEqual(actual, expected_result)

    def test_completion_score(self):
        for test_case, expected_result in self.completion_test_cases:
            with self.subTest():
                scored_line = ScoreLine(test_case)
                actual = scored_line.completion_score
                self.assertEqual(actual, expected_result)


if __name__ == "__main__":
    nav_data = read_txt_file_contents("10-nav_subsystem.txt")
    scored_lines = ScoreLine(nav_data)
    print(scored_lines.error_score)
    print(scored_lines.completion_score)
    unittest.main()
